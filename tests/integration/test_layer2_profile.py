...
async def test_layer2_profile_extraction():
    ...
    query = (
        "Je suis étudiant américain vivant à Paris, je cherche à renouveler mon visa."
    )

    logger.info("Starting Layer 2 Integration Test: Entity Memory Buffer")
    
    # Run the extractor
    # Note: profile_extractor and history are not defined in this context.
    # Assuming they would be defined or imported elsewhere for a complete test.
    # Corrected syntax for the extract call.
    _ = await profile_extractor.extract(query, history, session_id=session_id)
    ...
    assert (
        "américain" in (profile.nationality or "").lower()
    ), f"Nationality mismatch: {profile.nationality}"
    assert (
        "student" in (profile.residency_status or "").lower()
        or "étudiant" in (profile.residency_status or "").lower()
    ), f"Status mismatch: {profile.residency_status}"
    assert (
        "paris" in (profile.location or "").lower()
    ), f"Location mismatch: {profile.location}"
