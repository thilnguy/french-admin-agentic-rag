...
async def test_layer4_state_prompt():
    ...
    state_with_profile = AgentState(
        session_id="test_layer4",
        user_profile=UserProfile(nationality="Américaine")
    )
    ...
    with patch.object(procedure_agent, "_run_chain", side_effect=procedure_agent._run_chain) as mock_run_chain:
    ...
    with patch.object(procedure_agent, "_run_chain") as mock_run_chain:
        ...
        await procedure_agent._explain_procedure(query, mock_docs, state_with_profile.user_profile.model_dump())
        
        # Verify call args
        args, kwargs = mock_run_chain.call_args
        # args[0] is the chain, args[1] is input_data
        input_data = args[1] if len(args) > 1 else kwargs.get("input_data") # wait, _run_chain signature is (chain, input_data)
        
        # Check if input_data contains our profile
        assert "user_profile" in input_data, "user_profile missing from input_data"
        assert input_data["user_profile"]["nationality"] == "Américaine", "Profile nationality mismatch"
