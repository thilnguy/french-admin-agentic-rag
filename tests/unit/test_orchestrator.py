...
async def test_handle_query_cache_hit():
    ...
    with (
        ...
        # Mock Memory (State load happens BEFORE cache check)
        mock_state = AgentState(session_id="test", messages=[])
        mock_memory.load_agent_state = AsyncMock(return_value=mock_state)
        ...
        with (
            ...
            mock_memory.load_agent_state.assert_called_once()
...
async def test_handle_query_flow():
    ...
    with (
        ...
        # Mock Translator
        mock_translate.return_value = "Generated Answer"
...
async def test_orchestrator_cache_exceptions():
    with (
        ...
        patch("src.agents.orchestrator.retrieve_legal_info", new_callable=AsyncMock),
        patch(
            "src.shared.guardrails.guardrail_manager.check_hallucination",
            new_callable=AsyncMock,
            return_value=True,
        ...
        # Mock Redis correctly as AsyncMock
        mock_redis = AsyncMock()
