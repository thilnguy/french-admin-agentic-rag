...
### A. Linux (NVIDIA)
- **Tool**: [Unsloth](https://github.com/unslothai/unsloth) / [Axolotl](https://github.com/OpenAccess-AI-Collective/axolotl)
- **Execution**: `axolotl finetuning/axolotl_config.yml`
- **Adapters**: Saved to `finetuning/qwen-7b-french-admin-distilled`
- **Inference**: vLLM (`python -m vllm.entrypoints.openai.api_server`)

### B. Local (Mac M4 / Apple Silicon)
...
- **Inference**: MLX Server (`python -m mlx_lm.server`)
...
3.  Add the generated samples to your raw working file (e.g. `train_raw_v2_600.jsonl`).
4.  Run `python finetuning/scripts/format_for_training.py` to format the new raw dataset into ChatML.
5.  Re-run training based on your hardware (Axolotl for Linux or MLX script for Mac).
6.  Serve the model using your preferred inference engine (vLLM or MLX).
7.  Verify performance using `uv run python evals/runners/llm_judge.py`.
